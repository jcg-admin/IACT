...
class TestPermissionPrecedence:
    ...
    def test_permiso_directo_tiene_maxima_prioridad(self, django_assert_num_queries):
        ...
        user = User.objects.create_user(
            username="alice", password="segura123", email="alice@example.com"
        )
        ...
        role = user_models.Role.objects.create(
            name="Analista", description="Rol analista"
        )
        ...
        user_models.UserPermission.objects.create(
            user=user, permission=permission, granted_by=user
        )
    ...
    def test_permiso_por_rol_sin_permiso_directo(self):
        User = get_user_model()
        user = User.objects.create_user(
            username="carol", password="segura123", email="carol@example.com"
        )
        ...
        role = user_models.Role.objects.create(
            name="Auditor", description="Rol auditor"
        )
        role.permissions.add(permission)
