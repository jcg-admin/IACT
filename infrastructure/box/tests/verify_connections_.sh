...
# Sin emojis - Sin dependencias externas - Idempotente - Sin fallas silenciosas
...
MARIADB_PORT=$(ss -tlnp | grep mysqld | grep -oE '[0-9]+(?=/)' | head -n 1)
POSTGRES_PORT=$(ss -tlnp | grep postgres | grep -oE '[0-9]+(?=/)' | head -n 1)
