---
id: TASK-017-layer3-infraestructura-logs
...
```bash
# Copiar service file
sudo cp scripts/logging/collectors/infraestructura-log-collector.service \
 /etc/systemd/system/
...
# Habilitar servicio
sudo systemctl enable infraestructura-log-collector

# Iniciar servicio
sudo systemctl start infraestructura-log-collector

# Verificar estado
sudo systemctl status infraestructura-log-collector
```
...
systemctl status infraestructura-log-collector
...
```bash
# Logs del daemon
journalctl -u infraestructura-log-collector -n 50
