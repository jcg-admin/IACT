...
sudo systemctl stop gunicorn-iact-production
```
...
# Restore from backup
mysql -h $DB_HOST -u $DB_USER -p$DB_PASSWORD iact_production < backup_YYYYMMDD_HHMMSS.sql
...
sudo systemctl start gunicorn-iact-production
