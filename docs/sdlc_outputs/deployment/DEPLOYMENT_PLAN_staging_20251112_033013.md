...
Deploy feature: Unknown Feature to staging environment
...
git tag -a pre-deploy-$(date +%Y%m%d-%H%M%S) -m "Backup before Unknown Feature deployment"
...
sudo systemctl status gunicorn-iact-staging
...
# Check session table
mysql -h $DB_HOST -u $DB_USER -p$DB_PASSWORD -e "SELECT COUNT(*) FROM django_session;" iact_staging
```
