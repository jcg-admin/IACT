#!/bin/bash
#
# check-all.sh - Ejecuta todos los checks de calidad localmente
#
# Uso:
#   ./scripts/dev/check-all.sh
#   ./scripts/dev/check-all.sh --fix  # Intenta auto-corregir issues
#
# Descripción:
...
#   - Pre-commit config: api/callcentersite/.pre-commit-config.yaml
#
...
run_pre_commit_hooks() {
    ...
    if [[ ! -f "${pre_commit_dir}/.pre-commit-config.yaml" ]]; then
        echo -e "${YELLOW}ADVERTENCIA: No se encontró .pre-commit-config.yaml en ${pre_commit_dir}${NC}"
        return 0
    fi
...
validate_specs() {
    ...
    if [[ ! -f "scripts/dev/validate-spec.sh" ]]; then
        echo -e "${YELLOW}ADVERTENCIA: Script validate-spec.sh no encontrado${NC}"
        return 0
    fi
