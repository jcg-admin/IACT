...
```bash
python3 scripts/gobernanza_sdlc/automation/business_rules_validator_agent.py \
...
```bash
python3 scripts/gobernanza_sdlc/automation/compliance_validator_agent.py \
...
```python
from scripts.gobernanza_sdlc.automation.pdca_agent import PDCAAgent
...
## Notas de migración
- El contenido completo de los agentes se movió desde `scripts/coding/ai/automation/` a esta ruta. Las antiguas ubicaciones quedaron como _stubs_ que reexportan los módulos para mantener compatibilidad.
- El volumen de líneas eliminadas (≈6,2k) en la migración corresponde a la sustitución de los archivos anteriores por stubs mínimos; no se perdió lógica, solo se reubicó.
- Ejecute y mantenga las pruebas desde esta ruta para evitar divergencias entre los stubs y el código real.
