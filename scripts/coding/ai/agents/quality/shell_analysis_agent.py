...
class ShellScriptAnalysisAgent(Agent):
    ...
    def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif isinstance(script_paths, list):
            # Convert any strings in list to Path
            script_paths = [Path(p) if isinstance(p, str) else p for p in script_paths]
        ...
        return {
            ...
            "summary": summary,
            # Metadata for guardrails compliance
            "_metadata": {
                "description": """
                Shell Script Analysis Results

                Args:
                    script_paths: Paths to analyzed scripts

                Returns:
                    Dictionary containing analysis results and summary
                """,
                "traceability": "FEATURE-SHELL-ANALYSIS-001, ADR-2025-001"
            }
        }
